...
public class Cancel extends JFrame implements ActionListener {
    ...
    public Cancel() {
        ...
        lblflight.setBounds(60, 230, 150, 25);
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == fetchButton) {
            ...
            try {
                Conn conn = new Conn();
                // Selects name, flight code (or flight name), and travel date from booking based on PNR
                String query = "select * from booking where PNR = '"+pnr+"'";
                ResultSet rs = conn.s.executeQuery(query);
                
                if (rs.next()) {
                    tfname.setText(rs.getString("fname") + " " + rs.getString("lname"));
                    lblfcode.setText(rs.getString("flight"));
                    // Assuming your booking table has a 'dd' or 'date' column for travel date
                    // If not present in your specific schema, this might need adjustment
                    try {
                         lbldateoftravel.setText(rs.getString("dd")); 
                    } catch (Exception e) {
                         lbldateoftravel.setText("N/A");
                    }
                } else {
